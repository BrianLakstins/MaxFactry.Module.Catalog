// <copyright file="MaxClientViewModel.cs" company="Lakstins Family, LLC">
...
namespace MaxFactry.Module.Catalog.PresentationLayer
{
    ...
    /// </summary>
    public class MaxClientViewModel : MaxFactry.Base.PresentationLayer.MaxBaseIdViewModel
    {
        /// <summary>
        /// Internal storage of a sorted list.
        /// Can use Generic List if supported in the framework.
        /// </summary>
        private List<MaxClientViewModel> _oSortedList = null;

        /// <summary>
        /// Initializes a new instance of the MaxClientViewModel class
        /// </summary>
        public MaxClientViewModel()
            : base()
        {
        }
        ...
        public MaxClientViewModel(MaxEntity loEntity)
            : base(loEntity)
        {
        }
        ...
        public MaxClientViewModel(string lsId) : base(lsId)
        {
        }

        protected override void CreateEntity()
        {
            this.Entity = MaxClientEntity.Create();
        }
        ...
        public List<MaxClientViewModel> GetSortedList()
        {
            if (null == this._oSortedList)
            {
                this._oSortedList = new List<MaxClientViewModel>();
                string[] laKey = this.EntityIndex.GetSortedKeyList();
                for (int lnK = 0; lnK < laKey.Length; lnK++)
                {
                    MaxClientViewModel loViewModel = new MaxClientViewModel(this.EntityIndex[laKey[lnK]] as MaxEntity);
                    loViewModel.Load();
        ...
        protected override bool MapToEntity()
        {
            if (base.MapToEntity())
            {
                MaxClientEntity loEntity = this.Entity as MaxClientEntity;
                if (null != loEntity)
                {
