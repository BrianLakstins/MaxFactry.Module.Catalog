...
namespace MaxFactry.Module.Catalog.BusinessLayer
{
    ...
    public class MaxCatalogLibrary : MaxFactry.Core.MaxMultipleFactory
    {
        /// <summary>
        /// Internal storage of single object
        /// </summary>
        private static MaxCatalogLibrary _oInstance = null;
        ...
        /// </summary>
        public static MaxCatalogLibrary Instance
        {
            get
            {
                if (null == _oInstance)
                {
                    lock (_oLock)
                    {
                        if (null == _oInstance)
                        {
                            _oInstance = new MaxCatalogLibrary();
                        }
                    }
                }
        ...
        public static bool UpdateCrm(MaxOrderEntity loOrder)
        {
            IMaxProvider[] loList = Instance.GetProviderList();
            bool lbR = false;
            for (int lnP = 0; lnP < loList.Length; lnP++)
            {
                if (loList[lnP] is IMaxCrmLibraryProvider)
                {
                    if (!lbR)
                    {
                        lbR = ((IMaxCrmLibraryProvider)loList[lnP]).UpdateCrm(loOrder);
                    }
                }
            }
        ...
        public static bool ProcessTransaction(MaxOrderPaymentTransactionEntity loTransaction)
        {
            IMaxProvider[] loList = Instance.GetProviderList();
            bool lbR = false;
            for (int lnP = 0; lnP < loList.Length; lnP++)
            {
                if (loList[lnP] is IMaxPaymentLibraryProvider)
                {
                    if (!lbR)
                    {
                        lbR = ((IMaxPaymentLibraryProvider)loList[lnP]).ProcessTransaction(loTransaction);
                    }
                }
            }
